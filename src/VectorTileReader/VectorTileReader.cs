		...
		private VectorTileLayer getLayer(byte[] data) {
			...
			if (_Validate) {
				...
				if (layer.Keys.Count != layer.Keys.Distinct().Count()) {
					...
					string keysTxt = string.Join(
						System.Environment.NewLine
						, keys
					);
					// actually not an error.
					// duplicate keys are allowed but strongly discouraged.
					throw new System.Exception(
						string.Format(
							"Layer [{1}], duplicate key values found:{0}{2}"
							, System.Environment.NewLine
							, layer.Name
							, keysTxt
						)
				...
				if (layer.Values.Count != layer.Values.Distinct().Count()) {
					string[] vals =layer.Values
						...
						.ToArray()
					;
					string valsTxt = string.Join(System.Environment.NewLine, vals);
					throw new System.Exception(
						string.Format(
							"Layer [{1}], duplicate attribute values found:{0}{2}"
							, System.Environment.NewLine
							, layer.Name
							, valsTxt
						)
